...
dynamic deserializeParam<T>(
  ...
  try {
    ...
    if (isList) {
      ...
      return paramValues.where((p) => p is String).map((p) => p as String).map((p) => deserializeParam(p, paramType, false, collectionName)).where((p) => p != null).map((p) => p! as T).toList();
    }
    switch (paramType) {
      case ParamType.int:
...
Future<dynamic> Function(String) getDoc(
  ...
  return (String id) => FirebaseFirestore.instance.doc('$collectionName/$id').get().then((s) => serializers.deserializeWith(serializer, serializedData(s)));
}
...
Future<List<T>> Function(String) getDocList<T>(
  ...
  return (String idsList) {
    ...
    return Future.wait(
      docIds.map(
        (id) => FirebaseFirestore.instance.doc('$collectionName/$id').get().then(
